...
│   ├── __init__.py      # 路由包初始化
│   ├── user/            # 用户相关 API
│   │   ├── create.py    # 用户创建、注册、登录 ✨统一响应格式
│   │   ├── read.py      # 用户查询
│   │   ├── update.py    # 用户更新、密码更新、手机号密码重置 ✨统一响应格式
│   │   └── delete.py    # 用户删除
│   ├── posts/           # 帖子相关 API ✨新增
│   │   ├── create.py    # 帖子创建 ✨统一响应格式
│   │   ├── read.py      # 帖子查询、筛选
│   │   ├── update.py    # 帖子更新、点赞功能
│   │   ├── likes.py     # 点赞查询功能
│   │   └── delete.py    # 帖子删除
...
│   │   ├── update.py    # 横幅更新、统计
│   │   └── delete.py    # 横幅删除
│   ├── charts/          # 图表相关 API ✨统一响应格式
│   ├── forum/           # 论坛相关 API
│   ├── image/           # 图片相关 API
│   ├── history/         # 历史记录相关 API ✨统一响应格式
│   ├── releases/        # 发布版本相关 API
│   └── sms/             # 短信服务 API ✨新增
│       ├── sendsms.py   # 发送短信验证码 ✨统一响应格式
│       └── verifysms.py # 验证短信验证码 ✨统一响应格式
├── scripts/             # 数据库迁移脚本
├── examples/            # 使用示例和文档
...
├── utils/               # 工具类目录 ✨新增
│   ├── __init__.py      # 工具包初始化
│   ├── response.py      # 统一响应格式工具类 ✨新增
│   └── response_examples.md # 响应格式使用示例 ✨新增
├── uploads/             # 文件上传目录
│   ├── images/          # 图片文件存储
│   └── apk/             # APK文件存储
├── instance/            # 实例配置和数据库
...
所有API接口都使用统一的响应格式，确保前端处理的一致性。采用智能的 `code`、`message`、`data` 三字段格式：
