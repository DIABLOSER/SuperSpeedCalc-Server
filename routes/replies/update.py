...
def update_reply(reply_id):
    ...
    try:
        ...
        if not user_id or user_id != reply.user:
            return internal_error_response(message='Permission denied. Only the author can update this reply.'
            , code=403)
        
        # 检查帖子是否可见（防止在帖子被隐藏后仍能编辑评论）
        if not reply.post_ref.is_visible_to_user(user_id):
            return internal_error_response(message='Cannot edit reply on invisible or unapproved post'
            , code=403)
